...
1. El archivo `.env` puede estar en la **raíz del proyecto** (`gusano/.env`) o en `backend/.env`. Se carga primero el de la raíz y luego el de backend (este último puede sobrescribir).
2. Si no tienes `.env`, crea uno a partir de `.env.example` en la raíz o en `backend/`.
...
- `POST /api/scans` - Crear escaneo (token opcional: sin token → usuario demo)
...
- `GET /api/scans` - Listar escaneos del usuario (token opcional → demo)
- `GET /api/scans/:id` - Obtener escaneo específico (token opcional → demo)
